...
// GET /api/admin/reports/monthly-profit?year=2026
// Gross profit for each of the 12 months of the given year, for the bar
// chart at the bottom of the Reports tab. Uses the same definition as
// computeProfits() on the frontend (paid revenue minus labor cost minus
// Stripe/platform fees) so the chart always agrees with the stat bubbles
// above it. Built as a single grouped query per metric rather than looping
// 12 separate reports/summary calls -- months with no activity still show
// up (as 0) via the generate_series/LEFT JOIN rather than being skipped.
  ...
  try {
    ...
    const paidResult = await db.query(
      `SELECT EXTRACT(MONTH FROM paid_at)::int AS month,
              ...
              COALESCE(SUM(COALESCE(platform_fee, 0)), 0) AS platform_fee_total
       FROM invoices
       WHERE company_id = $1 AND status = 'paid' AND EXTRACT(YEAR FROM paid_at) = $2
       GROUP BY 1`,
      [req.companyId, year]
    );

    const laborResult = await db.query(
      `SELECT EXTRACT(MONTH FROM d.clock_in)::int AS month,
              COALESCE(SUM((d.worked_seconds / 3600.0) * COALESCE(e.hourly_rate, 0)), 0) AS labor_cost
       FROM time_entry_durations d
       JOIN employees e ON e.id = d.employee_id
       WHERE e.company_id = $1 AND EXTRACT(YEAR FROM d.clock_in) = $2
       GROUP BY 1`,
      [req.companyId, year]
    );

    const paidByMonth = {};
    paidResult.rows.forEach(function(r) { paidByMonth[r.month] = r; });
    const laborByMonth = {};
    laborResult.rows.forEach(function(r) { laborByMonth[r.month] = r; });

    const months = [];
    for (let m = 1; m <= 12; m++) {
      const paidRow = paidByMonth[m];
      const laborRow = laborByMonth[m];
      const paidTotal = paidRow ? Number(paidRow.paid_total) : 0;
      const feesTotal = paidRow ? Number(paidRow.stripe_fee_total) + Number(paidRow.platform_fee_total) : 0;
      const laborCost = laborRow ? Number(laborRow.labor_cost) : 0;
      months.push({
        month: m,
        paid_total: paidTotal,
        labor_cost: laborCost,
        fees_total: feesTotal,
        gross_profit: paidTotal - laborCost - feesTotal,
      });
    }
